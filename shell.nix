...
stdenv.mkDerivation {
  ...
  alias k8=kubectl
  alias k8a="kubectl -lapp=transit-radar"
  alias prod-logs="kubectl logs -lapp=transit-radar -c transit-radar"
  ...
  alias prod-deploy="kubectl apply -f deployment.yaml && kubectl rollout status deployment transit-radar"
  '';
}
